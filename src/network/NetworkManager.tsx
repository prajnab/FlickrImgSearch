...
export class NetworkManager {
	...
	onError = async (error: AxiosError<ErrorResponse>) => {
		...
		if (error.response) {
			...
			title = Strings.errorMessage._oops;
			message =
				errorResult.data?.message ||
				Strings.errorMessage._something_went_wrong;
		} else if (error.request) {
			errorResult = error.request;
			Object.consoleLog("Error Result:", errorResult);
			if (error.message === "Network Error") {
				// Issue with accessing internet even thought phone is connected to network (Eg. Firewall)
				title = Strings.errorMessage._no_internet;
				message = Strings.errorMessage._no_network_desc;
			} else {
				// The request was made but no response was received
				title = Strings.errorMessage._oops;
				message = Strings.errorMessage._something_went_wrong;
			}
		} else {
